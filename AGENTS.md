...
- Do not silently discard fallible Rust daemon operations. Do not use `let _ = ...`, `.ok()`, or equivalent to ignore errors from network I/O, resolver calls, socket sends, netlink, routing, firewall, fd, process, or cleanup operations. Propagate the error or log it with useful context; if an operation is intentionally best-effort, handle only the expected benign failure mode explicitly.

...
| The dataplane child | `ProcessBuilder` at the app UID, holding a duplicate of the TUN descriptor received over the bootstrap socket; no Shizuku identity is involved | close the control socket, then SIGTERM, then SIGKILL on the launched child PID, waiting for observed exit | control-socket EOF makes a healthy child exit; nothing signals a wedged one |

...
//! session, in [crate::output], reached by every producer - and puts every TUN write behind one owner. Two
//! mappings from the same client can share a reassembly tuple, so a per-mapping allocator would hand the
//! same value to both.
//!
//! A mapping is reachable only through [crate::workers], which is what makes the refund honest:
...
use crate::egress::{self, Fragmentation};
...
use crate::report;
use crate::send_failure::{self, Failure};
...
/// can observe, which is what the Resource Policy requires of optional state - and the plan's global
/// oldest-first eviction ladder is not needed, because there is no shared pool to run out of.
...
impl Mapping {
    ...
    /// [crate::workers::Held] row in [Relay::mappings], whose own charge already counts
...
impl Relay {
    ...
    pub(crate) fn relay(
        ...
        // one rather than named separately: neither is forwardable, and answering either is ICMP's job in a
        // later slice.
...
mod tests {
    ...
    /// One mapping, built from the fields [Relay::open] builds it from, with an ordinary host UDP socket in
    /// place of the selected-network one that call cannot make on a host.
    ///
    /// The lease is deliberately bytes-only - [Mapping::footprint], the state a mapping owns - rather than the
    /// composite [Mapping::first_send] grant production takes. What that leaves out is the record side: the
    /// mapping's own record and the one per permitted remote, which the relay grows and shrinks around the
    /// calls below. Tests that own the ledger prove that half; this fixture exists so the filter's own rule
    /// can be driven at all, and it stays chargeable enough for the drop-before-refund ordering to mean
    /// something.
    fn mapping(admission: &mut Admission, domain: socket2::Domain) -> Mapping {
        let socket =
            Socket::new(domain, socket2::Type::DGRAM, None).expect("a host datagram socket");
        socket.set_nonblocking(true).expect("nonblocking");
        let lease = admission
            .reserve(Request::bytes(
                Mapping::footprint().expect("chargeable"),
                Class::General,
            ))
            .expect("granted");
        let (commit, _held) = oneshot::channel();
        Mapping {
            state: State::Live,
            commit: Some(commit),
            socket: Arc::new(AsyncFd::new(socket).expect("registered")),
            remotes: HashMap::with_capacity(REMOTES_PREPARED),
            deadline: Instant::now() + MAPPING_TIMEOUT,
            lease,
            history: History::with_capacity(HISTORY_DEPTH),
    ...
    fn datagram(payload: &[u8]) -> Relayed<'_> {
        Relayed {
            source: SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::new(192, 0, 2, 2), 40_000)),
            destination: SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::new(198, 51, 100, 7), 4_000)),
            hop_limit: 64,
            dont_fragment: false,
            payload,
        }
    ...
    /// A socket that refuses the daemon's own fragmentation option is described, not merely counted.
    ///
    /// `set_fragmentation` is a `setsockopt` on a socket this daemon opened, so nothing a client sends can
    /// make it fail: every failure is the daemon's own, and `df_failed` on its own tells the app only that
    /// some datagram did not leave. The datagram is still dropped - sending it under a fragmentation policy
    /// the kernel did not accept is exactly what must not happen - and the report is what says why.
    ///
    /// The refusal is produced by giving the mapping a `AF_UNIX` datagram socket, which cannot take an
    /// `IPPROTO_IP` option at all. That is also the only way to reach this branch from a host: the first-send
    /// path opens its socket through `android_setsocknetwork`, which a test build refuses.
    #[tokio::test]
    async fn a_socket_refusing_the_fragmentation_option_is_reported_once() {
        ...
        let (mut relay, _events) = Relay::new(&mut admission).expect("a relay");
        ...
        let record = mapping(&mut admission, socket2::Domain::UNIX);
        let identity = crate::workers::Identity {
            id: 1,
            cancel: tokio_util::sync::CancellationToken::new(),
        };
        relay
            .mappings
            .admit(sent.source, &identity, record, std::future::pending())
            .unwrap_or_else(|_| panic!("the fixture mapping is the only one"));
        ...
        let collector = tokio::spawn(async move {
            ...
            while let Some(message) = published.recv().await {
                ...
                if report.context == "shizuku.udp_fragmentation" {
                    seen.push(report);
                }
            }
            seen
        ...
        relay.relay(&[], sent, &gateways, &mut output, &mut admission);
        assert_eq!(1, relay.counters.df_failed);
        assert_eq!(
            0, relay.counters.sent,
            "nothing may leave under a policy the kernel refused"
        );
        ...
        assert_eq!(
            1,
            reports.len(),
            "exactly one report for one refusal: {reports:?}"
        );
        assert!(
            reports[0].errno.is_some(),
            "the report carries the kernel's own code: {:?}",
            reports[0]
        );
        let details: Vec<_> = reports[0]
            .details
            .iter()
            .map(|detail| (detail.key.as_str(), detail.value.as_str()))
            .collect();
        assert!(
            details.contains(&("destination", "198.51.100.7:4000")),
            "{details:?}"
        );
    }

    /// A refusal with no path MTU behind it is counted and stays silent.
    ///
    /// The other half of the fragmentation test above, and the more important half to pin: everything on
    /// this branch is client-drivable - the datagram's size is the client's choice - so turning it into a
    /// report would hand any local app a report stream. `EMSGSIZE` classifies as `TooBig`, the error queue
    ...
    async fn an_oversized_datagram_with_no_attributable_mtu_is_counted_not_reported() {
        ...
        let oversized = Relayed {
            source: SocketAddr::V6(std::net::SocketAddrV6::new(
                "2001:db8::2".parse().expect("a documentation address"),
                40_000,
                0,
                0,
            )),
            destination: SocketAddr::V6(std::net::SocketAddrV6::new(
                "2001:db8::7".parse().expect("a documentation address"),
                4_000,
                0,
                0,
            )),
        ...
        let record = mapping(&mut admission, socket2::Domain::IPV6);
        let identity = crate::workers::Identity {
            id: 1,
            cancel: tokio_util::sync::CancellationToken::new(),
        };
        relay
            .mappings
            .admit(oversized.source, &identity, record, std::future::pending())
            .unwrap_or_else(|_| panic!("the fixture mapping is the only one"));
        ...
        let collector = tokio::spawn(async move {
            ...
            while let Some(message) = published.recv().await {
                ...
                if report.context.starts_with("shizuku.udp") {
                    seen.push(report.context);
                }
            }
            seen
        });
        ...
        assert!(
            ...
            "a size the client chose is never a report"
        );
    }

    /// A config that names a network but no interface opens no relay work at all.
    ///
    /// The counter is the assertion, not the decoded field: an unconnected reply is demultiplexed on local
    /// address and port alone, so the arrival interface is the only thing separating a late reply to a retired
    /// mapping from a reply to the mapping that reused its port. Without it there is nothing to check, so the
    /// relay refuses the operation rather than serving it unchecked - while the resolver and the terminating
    /// TCP engine, which connect, carry on.
    #[tokio::test]
    async fn a_network_without_an_interface_opens_no_relay_work() {
        let mut admission = admission();
        let (mut relay, events) = Relay::new(&mut admission).expect("a relay");
        let gateways = Gateways::new();
        let (writer, _queue, _terminals) = tun_writer::channel();
        let mut output = Output::testing(1_500, 8, writer);
        ...
        relay
            .apply(
                Stamp::default(),
                selected_only.relay_upstream,
                &mut admission,
            )
            .await;
        let before = admission.records_charged();
        relay.relay(
            &[],
            datagram(b"hello"),
            &gateways,
        ...
        assert_eq!(
            relay.counters.no_upstream, 1,
            "the operation was refused for want of an interface"
        );
        assert!(relay.mappings.is_empty(), "and no mapping was opened");
        assert!(!relay.mappings.working(), "nor any worker started");
        assert_eq!(
            admission.records_charged(),
            before,
            "nothing was charged for work that did not happen"
        );

        // Neither half present is the same answer, and for the same reason.
        let nothing = egress::decode(&config(None, None)).expect("a real shape");
        relay
            .apply(Stamp::default(), nothing.relay_upstream, &mut admission)
            .await;
        relay.relay(
            &[],
            datagram(b"hello"),
        ...
        assert_eq!(relay.counters.no_upstream, 2);
        assert!(relay.mappings.is_empty());

        // With the interface, the relay gets past that refusal and really tries: the platform call is what
        // fails on a host, which is a different counter and a different fact.
        let whole = egress::decode(&config(Some(0x1234), Some(7))).expect("a real shape");
        assert!(whole.relay_upstream.is_some());
        relay
            .apply(Stamp::default(), whole.relay_upstream, &mut admission)
            .await;
        relay.relay(
        ...
        assert_eq!(
            relay.counters.no_upstream, 2,
            "the refusal for want of an interface did not happen again"
        );
    ...
    /// One mapping's reply filter admits its whole bound, refuses past it, reuses what expires, and is
    /// charged for the rows it may hold.
    ///
    /// Four claims and no more, each through a production call. [Mapping::permits],
    /// [Mapping::record_remote] and [Mapping::expire_remotes] are the filter's own admission rule - the same
    /// three [Relay::relay] and [Relay::sweep] call - so a change to the bound or to per-remote expiry fails
    /// here. [Mapping::footprint] is the row-state equation, asserted as an equality so a term that moved or
    /// got counted twice fails here too. The expiry is timed against a mapping that would still be live, which
    /// is the only case in which freed remote slots are reused at all. And the retirement at the end drops
    /// every physical owner before the lease is released, which is the ordering [Relay::close] keeps.
    ///
    /// What it deliberately does *not* cover is the record accounting the relay wraps those calls in: the
    /// `Admission::grow` a new remote costs and the `shrink` an expiry refunds. The lease here is bytes-only,
    /// held so the fixture is chargeable at all, and duplicating the ledger inside a filter test would be
    /// asserting the aggregate twice. That half is proved where it lives -
    /// `admission::tests::a_first_send_is_one_grant_and_every_precommit_failure_restores_it` for the composite
    /// grant and the per-remote record either way, and
    /// `a_network_without_an_interface_opens_no_relay_work` above for the relay refusing before it charges.
    ///
    /// What it cannot reach is [Relay::open]: that runs past a real `egress::open_udp`, and this binary's
    /// `android_setsocknetwork` is a stub that refuses in a test build, so no mapping is ever constructed
    /// through the relay on a host. The mapping below is therefore built directly, from an ordinary host UDP
    /// socket, with nothing Android-shaped in it.
    #[tokio::test]
    async fn a_reply_filter_admits_its_bound_reuses_expiries_and_is_charged_for_its_rows() {
        // The composite charge less every term that is not the filter, which leaves what the filter's rows
        // were charged. An equality rather than a bound, so it also pins what is *not* in the charge: the
        // `Mapping` header and the `History` header inside it are counted once, by the `Workers` row that
        // holds them, and adding either here would leave a remainder this assertion would fail on.
        let filter = Mapping::footprint()
            .expect("chargeable")
            .checked_sub(
                History::footprint(HISTORY_DEPTH).expect("chargeable")
                    // Named through the crate rather than the `egress` in scope here, which this test
                    // module's own import points at the shared one.
                    + crate::egress::ErrorQueue::footprint(),
            )
            .expect("the filter is one of the terms");
        assert_eq!(
            filter,
            REMOTES_PREPARED as u64 * std::mem::size_of::<(IpAddr, Instant)>() as u64,
            "the filter is charged for its rows, and the headers are not charged again"
        );

        // And the rule, through the mapping's own three calls.
        let mut admission = admission();
        let baseline = admission.bytes_charged();
        let mut mapping = mapping(&mut admission, socket2::Domain::IPV4);
        let remote = |index: u32| IpAddr::V4(Ipv4Addr::from((0x0a00_0000 + index).to_be_bytes()));

        // The whole bound, each destination checked before it would have been sent to.
        for index in 0..REMOTES_PREPARED as u32 {
            assert_eq!(
                mapping.permits(remote(index)),
                Permit::Free,
                "remote {index} is inside the bound"
            );
            mapping.record_remote(remote(index));
        }
        assert_eq!(mapping.remotes.len(), REMOTES_PREPARED);

        // One past it is refused, which is what stops the datagram before it leaves.
        let newcomer = IpAddr::V4(Ipv4Addr::new(203, 0, 113, 9));
        assert_eq!(
            mapping.permits(newcomer),
            Permit::Full,
            "a sixty-fifth destination is refused"
        );
        assert_eq!(
            mapping.remotes.len(),
            REMOTES_PREPARED,
            "atomically: asking recorded nothing"
        );

        // A destination already permitted is refreshed rather than added, so an established conversation is
        // never subject to the bound. A later send is what does that, and production does it in this order:
        // the mapping's own deadline moves first, then the remote is recorded under the new one.
        let opened = mapping.deadline;
        let refreshed = opened + MAPPING_TIMEOUT;
        assert_eq!(mapping.permits(remote(0)), Permit::Recorded);
        mapping.deadline = refreshed;
        mapping.record_remote(remote(0));
        assert_eq!(mapping.remotes.len(), REMOTES_PREPARED);

        // Now a sweep at a moment past the *remotes'* original deadlines but short of the mapping's refreshed
        // one, which is what [Relay::sweep] runs into on a mapping that is still carrying traffic: the mapping
        // itself is nowhere near expiry, and the destinations it stopped talking to go.
        let swept = opened + Duration::from_secs(1);
        assert!(
            mapping.deadline > swept,
            "the mapping would still be live at this sweep, so its remotes are what expire"
        );
        assert_eq!(
            mapping.expire_remotes(swept),
            REMOTES_PREPARED - 1,
            "every destination that went quiet expired"
        );
        assert_eq!(mapping.remotes.len(), 1, "and the refreshed one stayed");
        assert_eq!(
            mapping.permits(remote(0)),
            Permit::Recorded,
            "so the conversation that is still going carries on"
        );

        // The freed logical slots take successors, and the bound binds again at the same count.
        for index in REMOTES_PREPARED as u32..(2 * REMOTES_PREPARED - 1) as u32 {
            assert_eq!(
                mapping.permits(remote(index)),
                Permit::Free,
                "a freed slot takes destination {index}"
            );
            mapping.record_remote(remote(index));
        }
        assert_eq!(mapping.remotes.len(), REMOTES_PREPARED);
        ...
        // Retired the way [Relay::close] retires one: the lease comes out of the record and is released only
        // after everything the record owned has been dropped, so the budget is never told a descriptor is
        // gone while this scope still holds it.
        let Mapping {
            socket,
            remotes,
            lease,

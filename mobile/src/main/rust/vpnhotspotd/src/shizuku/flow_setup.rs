...
pub(crate) struct Sizing {
    /// The value the channels and buffers below are sized from.
...
pub(crate) struct Prepared {
    ...
    /// a filled query back. Built once per flow rather than allocated per question.
...
pub(crate) fn prepare<R>(
    ...
    // The reserved tail is that receive buffer, asked of the socket - see [bridge::TailCapacity].
    ...
    // An identity that cannot be issued is a flow that must not exist. The client-side socket is already in
    // hand, so this unwinds it here rather than leaving the transaction to do it.
    let Ok(identity) = workers.identity() else {
        sockets.remove(handle);
        return Err(Denied::Identity);
    };
    // A bounded stream pair carries steady-state traffic and a reserved tail extracts the client's ending.
    // Nothing of ours travels on either: the backpressure is the buffers filling and every wake is the
    ...
    // oneshot per query would add an allocation to every transaction.
    let (answers, control_end) = mpsc::channel(sizing.flow.control);
    let (filled, accepted) = mpsc::channel(sizing.flow.control);
    Ok(Prepared {
        handle,
        identity,
        bridge,
...
pub(crate) fn release(sockets: &mut Sockets, handle: SocketHandle, leftovers: Leftovers) {
    sockets.remove(handle);
    let Leftovers {
        bridge,
        stream,

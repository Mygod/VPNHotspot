...
use vpnhotspotd::shared::model::{
    ipv6_nat_gateway, ipv6_nat_prefix, Ipv6NatPorts, SessionConfig, SessionPorts, UpstreamConfig,
    UpstreamRole, DAEMON_ICMP_NFQUEUE_NUM, DAEMON_INTERCEPT_FWMARK_MASK,
...
impl IptablesChain {
    const fn new(target: IptablesTarget, table: &'static str, chain: &'static str) -> Self {
        Self {
            target,
            table,
            chain,
...
impl Ipv6NatFirewall {
    ...
    const NAT_MANGLE_CHAINS: [IptablesChain; 4] = [
    ...
    fn filter_jump_rules() -> Vec<IptablesRule> {
    ...
    fn prerouting_rule() -> IptablesRule {
        IptablesRule::new(
            IptablesTarget::Ipv6,
            "mangle",
            "PREROUTING",
...
impl Runtime {
    ...
    fn ipv6_block_jump_rules() -> Vec<IptablesRule> {
...
async fn delete_iptables_repeated(
    target: IptablesTarget,
    table: &'static str,
    chain: &'static str,
    args: &[&str],

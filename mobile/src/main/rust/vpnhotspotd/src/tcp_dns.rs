...
//!
//! The transport is the same shape as [crate::tcp_flow]'s splice and deliberately interchangeable with it: the
...
//! it (see [crate::tcp]'s retirement split), and the same connection therefore outlives the selection its
...
//! get their own SERVFAIL until a successor config supplies one - see [crate::tcp]'s flow source.
//!
...
use crate::tcp_flow::{hand_over_in_pieces, Chunk, Event, Handed, Mailbox, READ_CHUNK};
...
impl Delivered {
    ...
    /// anything. What it is for is the discard paths in [crate::tcp::Engine::settle]: a transport that is
    ...
    pub(crate) fn stamp(&self) -> crate::tun_writer::Stamp {
        self.stamp
    }
...
///
/// Both channels are built with the flow and charged with it - see [crate::tcp]'s per-flow footprint - which
...
/// the delivery terminal in [crate::virtual_dns], for the same reason - a balance cannot tell the two orders
...
/// and the client's own teardown finishes first; see [crate::tcp::terminal]. The resolver slot belongs to the

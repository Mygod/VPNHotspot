...
//! it sends through are [crate::echo_socket]'s, because they are retired by a different protocol.
...
use crate::echo_session::{Found, Sessions};
use crate::echo_socket::{Family, Refused, Sockets};
...
pub(crate) struct Relay {
    ...
    /// failure, for the same reason [crate::udp::Relay] holds one.
...
mod tests {
    ...
    fn request(payload: &[u8]) -> Request<'_> {
        Request {
            client: IpAddr::V4(Ipv4Addr::new(192, 0, 2, 2)),
            remote: IpAddr::V4(Ipv4Addr::new(198, 51, 100, 7)),
            hop_limit: 64,
            dont_fragment: false,
            identity: Identity {
                identifier: 1,
    ...
    /// A config that names a network but no interface opens no Echo session either, and for the same reason:
    /// an Echo Reply is demultiplexed on the ping socket's own identifier, so the arrival interface is the
    /// only thing separating a reply to this session from one to whatever reused it.
    #[tokio::test]
    async fn a_network_without_an_interface_opens_no_echo_work() {
        ...
        assert_eq!(relay.counters.no_upstream, 1);
        assert_eq!(relay.sessions.len(), 0, "no session was opened");
        assert_eq!(admission.records_charged(), before);
        ...
        assert_eq!(relay.sessions.len(), 0);

        // With the interface it gets past that refusal; what fails after it is the platform call.
    ...
    /// A nested constructor that refuses leaves nothing of this one charged.
    ///
    /// The order is what this is about: this owner reserves its own session table and *then* builds the socket
    /// owner, so a refusal there returns through a path that still holds a granted lease. A lease is an inert
    /// identity - dropping it releases nothing - so the early return has to release it explicitly or this
    /// owner's rows stay charged to a session that never started. Sized so the second reserve is the one that
    /// fails, which is what makes the first one's release the thing under test.
    #[tokio::test]
    async fn a_refused_nested_owner_leaves_this_ones_lease_released() {
        // Enough for the session table and not enough for the socket owner's reply channel behind it.
        let sessions = Sessions::footprint(SESSIONS_PREPARED).expect("chargeable")
            + crate::egress::ErrorQueue::footprint();
        let mut admission = Admission::new(Totals {
            admission_id: 1,
            record_total: 64,
            dns_record_floor: 4,
            byte_total: (1 << 20) + sessions + 4096,
            reserved_byte_floor: 1 << 20,
            fragment_cap: 1 << 16,
            dns_token_cap: 4,
            byte_only_owners: 8,
        ...
        let refused = Relay::new(&mut admission).err().expect("no relay fits");
        assert_eq!(refused, Denied::Bytes, "the nested owner is what refused");
        assert_eq!(
            admission.bytes_charged(),
            charged,
            "and this owner's own lease went back rather than being stranded"
        );

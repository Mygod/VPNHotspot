...
//!
//! Apart from the relay for the same reason [crate::echo_socket] is: this holds records and no descriptor, so it
...
impl Sessions {
    ...
    /// a slot to go in - see [crate::echo::Relay]. An expiry frees one for the next ping. The map's own
    ...
    /// Drops everything, returning how many went. Used by retirement, where every session belongs to an epoch
    /// whose client addresses no longer mean anything.
...
mod tests {
    ...
    const CLIENT: IpAddr = IpAddr::V4(Ipv4Addr::new(192, 168, 43, 12));

    fn remote(last: u8) -> IpAddr {
        IpAddr::V4(Ipv4Addr::new(198, 51, 100, last))
    }

    /// One outstanding ping recorded the way [crate::echo::Relay] records it: a sequence allocated, the bound
    /// checked, and only then the session inserted.
    fn record(sessions: &mut Sessions, remote: IpAddr, now: Instant) -> Option<u16> {
        let sequence = sessions.allocate(remote).expect("a free sequence");
        // The relay's own order: the check comes before the send, and the insertion after it.
        if !sessions.admits() {
            return None;
        }
        sessions.insert(
    ...
    /// The logical bound is the whole admission rule: it admits its whole count, refuses the next, and a
    /// session that is answered or times out frees its slot for the one after it.
    ///
    /// Driven through the production calls - [Sessions::admits], [Sessions::allocate], [Sessions::insert],
    /// [Sessions::take] and [Sessions::expire] - rather than through a rule restated here, so the test moves
    /// if the owner's does. Nothing reads the map's own capacity: what the container does with its backing is
    /// count-bounded overhead, and the bound is what policy actually enforces.
    #[test]
    fn the_bound_admits_its_count_refuses_the_next_and_reuses_a_freed_slot() {
        let prepared = 4usize;
        let mut sessions = Sessions::with_capacity(prepared);
        let now = Instant::now();

        // The whole bound, one outstanding ping at a time.
        let mut live = Vec::new();
        for index in 0..prepared {
            assert!(sessions.admits(), "session {index} is inside the bound");
            let peer = remote(index as u8);
            let sequence = record(&mut sessions, peer, now).expect("inside the bound");
            live.push((peer, sequence));
        }
        assert_eq!(sessions.len(), prepared);

        // One past it is refused before anything reaches the wire, and nothing is recorded.
        assert!(!sessions.admits(), "a full table refuses the next ping");
        let extra = remote(200);
        assert_eq!(
            record(&mut sessions, extra, now),
            None,
            "and the relay's own order refuses it rather than inserting"
        );
        assert_eq!(sessions.len(), prepared, "atomically: nothing was recorded");

        // A reply consumes its session, which frees the slot the next ping takes.
        let (answered, sequence) = live.remove(0);
        let session = sessions
            .take(answered, sequence)
            .expect("its reply arrived");
        assert_eq!(session.client, CLIENT);
        assert_eq!(sessions.len(), prepared - 1, "the slot came back");
        assert!(sessions.admits(), "so the next ping may be recorded");
        let sequence = record(&mut sessions, extra, now).expect("a freed slot is available again");
        live.push((extra, sequence));
        assert_eq!(sessions.len(), prepared);

        // And so does an expiry, which is the other way a session ends.
        assert!(!sessions.admits());
        let expired = now + ECHO_TIMEOUT + Duration::from_secs(1);
        assert_eq!(
            sessions.expire(expired),
            prepared,
            "every session timed out together"
        );
        assert_eq!(sessions.len(), 0);
        for index in 0..prepared {
            assert!(sessions.admits(), "slot {index} is free again");
            record(&mut sessions, remote(index as u8), expired).expect("reused after expiry");
        }
        assert_eq!(sessions.len(), prepared);
        assert!(!sessions.admits(), "and the bound still binds");

        // A reply from somewhere never sent to matches nothing, however full the table is.
        assert!(sessions.take(remote(250), 0).is_none());
        assert_eq!(sessions.len(), prepared);
    }
}
